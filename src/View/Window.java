...
public class Window extends javax.swing.JFrame{
    ...
    public Window(WeatherReport WR, DbManager data) {
        ...
        weatherReport.addObserver((o, arg) -> updateView());

        dataPollution = data;
        linegraph = new ViewPollution(dataPollution);
        jPanel_Polution.add(linegraph);

        dataPollution.addObserver((o, arg) -> linegraph.updateChart());

        Temp_TextField.setEditable(false);
        Min_TextField.setEditable(false);
        Max_TextField.setEditable(false);
        Lon_TextField.setEditable(false);
        Lat_TextField.setEditable(false);
    ...
    private void jButton_ActualiserActionPerformed(java.awt.event.ActionEvent evt) {
        jButton_Actualiser.setEnabled(false);

        loadData();

        // Réactiver le bouton après un certain délai ou à la fin de la mise à jour
        new Thread(() -> {
            try {
                Thread.sleep(2000); // Exemple d'attente (ajustez selon les besoins)
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            SwingUtilities.invokeLater(() -> jButton_Actualiser.setEnabled(true));
        }).start();
    }
